...
cmaes_UpdateDistribution( cmaes_t *t, const double *rgFunVal)
{
  ...
  //  printf("Test if function values are identical, escape flat fitness\n");
  ...
  //  printf("update function value history\n");
  ...
  //  printf("calculate xmean and rgBDz~N(0,C)\n");
  for (i = 0; i < N; ++i) {
  ...
  //  printf("calculate z := D^(-1) * B^(-1) * rgBDz into rgdTmp\n");
  for (i = 0; i < N; ++i) {
    if (!flgdiag)
      for (j = 0, sum = 0.; j < N; ++j)
  ...
  */
  ...
  //  printf("cumulation for sigma (ps) using B*z\n");
  for (i = 0; i < N; ++i) {
    if (!flgdiag)
      for (j = 0, sum = 0.; j < N; ++j)
        sum += t->B[i][j] * t->rgdTmp[j];
  ...
  /* calculate norm(ps)^2 */
  //  printf("calculate norm(ps)^2\n");
  for (i = 0, psxps = 0.; i < N; ++i)
    psxps += t->rgps[i] * t->rgps[i];

  /* cumulation for covariance matrix (pc) using B*D*z~N(0,C) */
  //  printf("cumulation for covariance matrix (pc) using B*D*z~N(0,C)\n");
  ...
  /* update of C  */
  //  printf("update of C\n");
    //cmaes_WriteToFile(t, "C", "C_before.dat");

  Adapt_C2(t, hsig);
    
    //cmaes_WriteToFile(t, "C", "C_after.dat");
